...
The rendering pipeline first receives the data from each object, and evaluates all vertices independently in a shader program. In this step, transformations are performed and light computations are stored for each vertex. Later, these vertices are combined into primitives like lines and triangles, and a rasterization step computes the pixel positions affected by each primitve, called fragments. Each fragment contains the interpolated attributes from its respective vertices. In the last step, each fragment is processed to compute the final color of each pixel in an additional shader program.
